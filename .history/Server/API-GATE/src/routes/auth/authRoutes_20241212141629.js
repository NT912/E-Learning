...
router.post("/signupadmin", async (req, res) => {
  try {
    // Thêm role "admin" vào dữ liệu trước khi gửi đến authentication service
    const signupData = {
      ...req.body, // Các dữ liệu khác từ client (ví dụ: email, password)
    ...
    // Gửi yêu cầu đến authentication service với dữ liệu đã được chỉnh sửa
    const response = await axios.post(
      `${AUTH_SERVICE_URL}/auth/signupadmin`,
      signupData
    );

    // Trả lại phản hồi cho client
    res.status(response.status).json(response.data);
  } catch (error) {
