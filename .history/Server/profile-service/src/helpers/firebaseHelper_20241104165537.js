...
const firebaseHelper = {
  ...
  /**
   * Tải ảnh đại diện người dùng lên Firebase Storage
   * @param {Object} avatarFile - File ảnh đại diện cần tải lên
   * @return {Promise<String>} - URL tải xuống của ảnh đã tải lên
   */
  uploadAvatarUser: async (avatarFile) => {
    try {
      const timestamp = Date.now();
      const fileName = `${timestamp}_${avatarFile.originalname}`;
      const storageRef = ref(storage, `User_Avatar/${fileName}`);
      await uploadBytes(storageRef, avatarFile.buffer);
      const downloadURL = await getDownloadURL(storageRef);
      return downloadURL;
    } catch (error) {
      console.error("Firebase Error uploading avatar:", error);
      throw new Error(message.avatar.uploadError.description.failFirebase);
    }
  },
