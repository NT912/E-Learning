...
const lessonController = {
  create: async (req, res) => {
    const { chapterID, title, description, isAllowDemo, period, orderNumber, userID } = req.body;
    ...
    try {
      const result = await lessonService.createLesson(userID, chapterID, title, description, isAllowDemo, period, orderNumber);
      sendResponse(res, true, message.lesson.creationSuccess.title, { lessonID: result });
    } catch (err) {
      console.error(err);
